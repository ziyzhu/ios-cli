...
| Recent logs | `logs --last 1m [--bundle <id>] [--predicate '<NSPredicate>']` |
| Stream logs | `logs --follow [--bundle <id>] [...]` (blocks; use sparingly) |
...
- `logs --bundle <id>` builds a predicate matching `subsystem == "<id>" OR processImagePath CONTAINS "<id>"`; AND-composed with any explicit `--predicate`.
...
`logs` (one-shot) returns `{"lines": ["...", ...]}`. `logs --follow` is the lone exception: it streams raw text to stdout, not JSON.
