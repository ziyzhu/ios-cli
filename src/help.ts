...
export const COMMANDS: Command[] = [
  ...
  {
    name: "drag", group: "INTERACT", summary: "press, move through points, release — one continuous touch",
    usage: "drag <x1> <y1> <x2> <y2> [x y ...] | drag --label|--role|--text|--id <s> <x2> <y2> [x y ...]",
    args: [
      { name: "x1", desc: "start x (omit the start pair when a matcher supplies it)" },
      { name: "y1", desc: "start y" },
      { name: "x2", desc: "next x" },
      { name: "y2", desc: "next y" },
      { name: "points", variadic: true, desc: "further x y waypoints; the last pair is the release point" },
    ],
    flags: [
      { name: "label", type: "string", desc: "start at a matched AX element (substring, case-insensitive)" },
      { name: "role", type: "string", desc: "start at a matched AX element" },
      { name: "text", type: "string", desc: "start at a matched AX element" },
      { name: "id", type: "string", desc: "start at a matched AX element (exact)" },
      { name: "wait", type: "string", metavar: "ms", desc: "poll up to N ms for the matcher to hit" },
      { name: "stable", type: "string", metavar: "ms", desc: "require a stable frame before starting" },
      { name: "press", type: "string", metavar: "s", desc: "hold at the start before moving (the lift for drag & drop)" },
      { name: "duration", type: "string", metavar: "s", default: "0.25", desc: "movement time per segment" },
      { name: "hold", type: "string", metavar: "s", desc: "pause at the final point before releasing" },
      { name: "delta", type: "string", metavar: "px", default: "10", desc: "distance between interpolated move events" },
      { name: "settle", type: "string", metavar: "ms", desc: "wait after releasing" },
    ],
    notes: "Unlike `swipe` (an atomic flick that always releases at the end), `drag` is one continuous touch you control: press, optionally hold to lift (`--press 0.5` for drag & drop / reorder), move through every waypoint, optionally pause (`--hold`) over a drop target or mid-gesture state, then release at the last pair. To release midway through a path, just end the coordinates there.",
  },
  {
